...
def plot_fig_3_timeseries(save: bool = True):
    ...
    for col_idx, model in enumerate(["finbert", "roberta"]):
        ...
        ax.plot(df["date"], df[is_col], 
                color="#2c6fad", linewidth=1.6, label="Introductory Statement",
                alpha=0.9)
        ax.plot(df["date"], df[qa_col], 
                color="#c0392b", linewidth=1.3, linestyle="--", label="Q&A",
                alpha=0.8)
        
        ax.axhline(0, color="#888", linewidth=0.8, linestyle=":", zorder=1)
        ax.set_ylabel("Net Sentiment Score", fontsize=10)
        ax.set_title(f"{model.upper()} — IS vs. Q&A", fontsize=10, pad=6)
    ...
    fig.suptitle("Prepared vs. Spontaneous Communication: Sentiment Dynamics (Time Series)", 
                 fontsize=11, y=1.02)
    plt.tight_layout()
...
def plot_fig_4_scatter(save: bool = True):
    ...
    for col_idx, model in enumerate(["finbert", "roberta"]):
        ...
        scatter = ax.scatter(
            ...
            s=28, alpha=0.65, cmap="viridis", edgecolors="white", linewidths=0.3,
        )
        
        # 45-stupňová čiara
        lims = [-0.4, 0.75] if model == "finbert" else [-0.7, 0.35]
        ax.plot(lims, lims, 'k--', alpha=0.4, linewidth=1.2, zorder=1,
        ...
        ax.set_xlabel("IS Sentiment", fontsize=10)
        ax.set_ylabel("Q&A Sentiment", fontsize=10)
        ax.set_title(f"{model.upper()} Scatter", fontsize=9.5, pad=4)
        ax.grid(alpha=0.25, linewidth=0.6)
        ...
        if col_idx == 1:
            ...
            cbar.set_label("Date", fontsize=8)
            cbar.ax.tick_params(labelsize=7)
            cbar_ticks = cbar.get_ticks()
            cbar.set_ticks(cbar_ticks[::2])
            cbar.set_ticklabels([mdates.num2date(t).year for t in cbar_ticks[::2]])

    fig.suptitle("Prepared vs. Spontaneous Communication: Sentiment Dynamics (Correlation)", 
                 fontsize=11, y=1.02)
    plt.tight_layout()
