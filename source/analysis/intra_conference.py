...
def binned_profile(x: np.ndarray, y: np.ndarray, n_bins: int = 20) -> tuple:
...
def plot_fig_3_8a(save: bool = True):
    fig, axes = plt.subplots(1, 2, figsize=(13, 5.5), gridspec_kw={"wspace": 0.28})
    ...
    ax.set_xlabel("Relative position within IS (0 = start, 1 = end)")
    ax.set_ylabel("Sentiment")
    ax.set_title("Introductory Statement\nSentiment Profile", pad=6)
    ...
    ax.axhline(0, color="#ddd", linewidth=0.7)
    ax.set_xlabel("Relative position within Q&A (0 = start, 1 = end)")
    ax.set_ylabel("Sentiment")
