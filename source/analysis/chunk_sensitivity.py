...
def plot_limit_comparison(save: bool = True):
    ...
    selected = [1, 50, 150, 350]
    
    fig, axes = plt.subplots(2, 1, figsize=(12, 6), sharex=True)
    
    # Panel A: All selected limits overlaid
    ax1 = axes[0]
    colors = {"1": "#e74c3c", "50": "#f39c12", "150": "#27ae60", "350": "#3498db"}
    alphas = {"1": 0.4, "50": 0.6, "150": 1.0, "350": 0.8}
    lws    = {"1": 0.9, "50": 1.2, "150": 2.0, "350": 1.5}
    ...
    ax1.set_ylabel("RoBERTa mean sentiment", fontsize=10)
    ax1.set_title("(A) Sentiment Time Series across Chunk Sizes", fontsize=10, pad=6)
    ax1.legend(loc="lower left", fontsize=8.5, ncol=4, framealpha=0.9)
    ax1.grid(alpha=0.25, linewidth=0.6)
    ax1.set_ylim(-0.7, 0.4)
    
    # Panel B: Difference from 150-word baseline (to highlight divergence)
    ax2 = axes[1]
    baseline = df["limit_150"]
    for limit in [1, 50, 350]:
        diff = df[f"limit_{limit}"] - baseline
        ax2.plot(
            df["date"], diff,
            label=f"{limit} − 150 words",
            color=colors[str(limit)],
            alpha=0.7,
            linewidth=1.3,
        )
    ...
    ax2.axhline(0, color="#888", linewidth=1.0, linestyle="-", zorder=1,
                label="150-word baseline")
    ax2.set_xlabel("Date", fontsize=10)
    ax2.set_ylabel("Δ Sentiment (vs. 150 words)", fontsize=10)
    ax2.set_title("(B) Deviation from 150-word Baseline", fontsize=10, pad=6)
    ax2.legend(loc="lower left", fontsize=8.5, ncol=4, framealpha=0.9)
...
def compute_divergence_stats():
    ...
    print("\n=== Divergence from 150-word baseline ===")
    for limit in [1, 50, 100, 200, 250, 300, 350]:
        diff = df[f"limit_{limit}"] - baseline
        mae  = diff.abs().mean()
