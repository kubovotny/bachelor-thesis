...
def plot_fig_3_9a(df: pd.DataFrame, save: bool = True):
    ...
    for row, pairs in enumerate([top_pairs, bot_pairs]):
        for col, (feat, label, color, target, ylabel) in enumerate(pairs):
            ...
            ax.plot(
                xfit, slope * xfit + intercept, color=color, linewidth=1.8, alpha=0.85
            )
...
def plot_fig_3_9b(df: pd.DataFrame, save: bool = True):
    ...
    fig, axes = plt.subplots(2, 1, figsize=(13, 7), gridspec_kw={"hspace": 0.35})
    ...
    for ax_idx, (ax, title) in enumerate(zip(axes, titles)):
        ...
        ax.set_ylabel("Pearson r (rolling 20-meeting)", fontsize=12)
    ...
    if save:
        fig.savefig(
            OUTPUT_DIR / "model/fig_rolling_correlation.pdf", bbox_inches="tight"
        )
...
if __name__ == "__main__":
    ...
    df = load_data()
    print(
        f"  {len(df)} meetings, {len([c for c in df.columns if c.startswith('d_')])} delta features"
    )
