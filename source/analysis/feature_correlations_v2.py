...
def load_data() -> pd.DataFrame:
    ...
    shadow_col = next(
        (c for c in df.columns
         if any(k in c.lower() for k in ("shadow", "wu", "xia"))),
        None,
    )
    ...
    lag_dict = {
        f"{col}:lag{lag}": (diff_dict.get(col, df.get(col, pd.Series()))
                            if col.startswith("d_") else df[col]).shift(lag)
        for col in all_base
...
def _get_feature_cols_clean(df: pd.DataFrame) -> list[str]:
    ...
    for c in df.columns:
        ...
        is_level_lag = (":lag" in c and
                        any(c.split(":lag")[0].endswith(sfx)
                            for sfx in base_ends))
        # Delta features
        is_delta = c.startswith("d_") and any(
            c.replace("d_", "").split(":lag")[0].endswith(sfx)
            for sfx in base_ends
        )
...
def compute_label_overlap(df: pd.DataFrame) -> pd.DataFrame:
    is_means = [
        c for c in df.columns
        if "IS" in c and c.endswith("_mean") and ":lag" not in c
    ...
    for i, c1 in enumerate(is_means):
        for c2 in is_means[i + 1:]:
            ...
            records.append({"Feature A": c1, "Feature B": c2,
                            "r": round(r, 3), "n": len(valid)})
    ...
    for era, (start, end) in ERAS.items():
        ...
        for feat in feat_cols:
            ...
            if len(valid) < MIN_OBS:
                continue
            if valid[feat].std() < MIN_FEAT_STD or valid[target_col].std() < MIN_FEAT_STD:
                continue
...
if __name__ == "__main__":
    print(f"Loading data (word_limit_unlabeled={WORD_LIMIT}, "
    ...
    piv_levels = pivot_summary(corr_df, require_full_coverage=True,
                               levels_only=True)
    piv_deltas = pivot_summary(corr_df, require_full_coverage=True,
                               deltas_only=True)
