...
CUSTOM_STOPWORDS = STOPWORDS | {
    # generic filler
    "will", "also", "said", "well", "one", "two", "think", "know",
    "would", "could", "may", "must", "need", "like", "make", "go",
    "come", "see", "say", "mean", "want", "looking", "continue",
    "question", "answer", "president", "vice", "mr", "ms", "thank",
    "sure", "right", "first", "second", "third", "let", "just",
    "now", "still", "already", "much", "many", "way", "part",
    "thing", "point", "terms", "kind", "fact", "sense",
    # ECB boilerplate
    "governing", "council", "ecb", "european", "central", "bank",
    "euro", "area", "eurozone", "member", "states",
    # names of ECB presidents / officials that skew per-era
    "draghi", "lagarde", "trichet", "duisenberg", "weidmann", "schnabel",
    "fazio", "solbes", "willem", "wim", "wiliem", "reynders", "rompuy",
    "juncker", "barroso", "monti", "pedro", "madrid", "singapore",
    "greenspan", "welteke", "noyer", "issing", "christian", "otmar",
    "liaison", "residents", "homework", "printing", "coins",
    # country names that appear due to membership events, not substance
    "latvian", "latvia", "estonian", "estonia", "croatian", "croatia",
    "slovak", "slovenian", "lithuanian",
}
...
def load_data(model: str = MODEL) -> pd.DataFrame:
    df = return_sentiment_chunk_data(limit_version=200, with_label=False)
    df = df[df["sentiment_model"] == model].copy()
    df["date"] = pd.to_datetime(df["date"])
    return df

# ── Relative frequency weighting ─────────────────────────────────────────────
def relative_freq(era_count: Counter, background_count: Counter, n_era: int, n_bg: int) -> dict:
    """
...
def polarity_colors(era_df: pd.DataFrame, bg_df: pd.DataFrame) -> dict:
    ...
    def hawk_ratio(df):
        ...
        return {w: (hawk.get(w,0) - dove.get(w,0)) / max(hawk.get(w,0) + dove.get(w,0), 1)
                for w in all_words}
...
def plot_fig_3_3(save: bool = True) -> plt.Figure:
    ...
    for row, (model, model_label) in enumerate(models):
        ...
        for col, (era_label, (start, end)) in enumerate(era_list):
            ...
            if row == 0:
                ax.set_title(era_label, fontsize=8.5, fontfamily="serif",
                             pad=4, color="#222222")

            # Model labels on left column only
            if col == 0:
                ax.set_ylabel(model_label, fontsize=9, fontfamily="serif",
                              rotation=90, labelpad=6, va="center", color="#333333")
    ...
    if save:
        path = OUTPUT_DIR / "fig_3_3_era_vocab_comparison.pdf"
