...
This chapter outlines the~data collection framework and the~implementation of NLP and Machine Learning. The~empirical foundation of this methodology rests upon data scraping and preprocessing.
...
Once the segments are labeled, the next step involves extracting the actual emotional tone (the sentiment) from the text. We decided not to rely on just one algorithm. Different models can pick up on different nuances. Instead, we use a two models paralleling approach (FinBERT and CentralBankRoBERTa) to evaluate every chunk in both the \gls{is} and \gls{qa} datasets.

The choice of these two models is intentional. FinBERT is the industry standard for general financial text. Whereas CentralBankRoBERTa is specifically tuned to recognize the unique language of central bankers. It understands categories like ``Hawkish'' (signaling tighter policy) and ``Dovish'' (signaling looser policy), which are far more useful for our analysis than simple positive or negative labels. For both models, we calculate a net sentiment score for each chunk using the following formula:
...
that reduces the overall intensity of the signal. If a press conference is filled with neutral filler text, the final score should naturally gravitate toward zero.
...
To demonstrate how these models process the ECB's language in practice, we provide a few illustrative examples in Table \ref{tab:sentiment_examples}. These segments show how FinBERT and CentralBankRoBERTa assign different weights to the same sentence, which justifies our choice to use both. For instance, a sentence about stagnant growth might be seen as purely ``Negative'' by FinBERT, while CentralBankRoBERTa might interpret it as a ``Dovish'' signal for future interest rates.
